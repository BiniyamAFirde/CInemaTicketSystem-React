...
namespace CinemaTicketSystem.Controllers
{
    ...
    public class BookingController : Controller
    {
                ...
                await strategy.ExecuteAsync(async () =>
                {
                    using (var transaction = await _context.Database.BeginTransactionAsync())
                    {
                        try
                        {
                            ...
                            // 🔄 ATOMIC OPERATION: Set RowVersion for optimistic concurrency control
                            foreach (var seat in seatsToBook)
                            {
                                // Store the current RowVersion as the original value
                                // This tells EF Core to include it in the WHERE clause of the UPDATE
                                _context.Entry(seat).Property(s => s.RowVersion)
                                    .OriginalValue = seat.RowVersion;
                            }
                            ...
                            // 🔄 ATOMIC SAVE: This will generate SQL with RowVersion in WHERE clause
                            // UPDATE Seats SET Status='Booked', RowVersion=<new> 
        ...
        private async Task<IActionResult> RedisplayForm(BookingViewModel model)
        {
            ...
            var selectedSeatIds = new HashSet<int>(model.SelectedSeatIds ?? new List<int>());
            model.Seats = screening.Seats
                ...
                .Select(s => new SeatViewModel
                {
                    ...
                    Status = s.Status.ToString(),
                    RowVersion = s.RowVersion,
                    IsSelected = selectedSeatIds.Contains(s.Id)
                })
                .ToList();
        ...
        public async Task<IActionResult> Cancel(int id)
        {
            ...
            // Return the booking directly to the view, which will handle display
            // The Cancel view will use the Booking model directly
            return View(booking);
        }
        ...
        [ActionName("Cancel")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> CancelConfirmed(int id, byte[]? rowVersion)
        {
            var user = await _userManager.GetUserAsync(User);
            if (user == null)
            {
                ...
                await strategy.ExecuteAsync(async () =>
                {
                    using (var transaction = await _context.Database.BeginTransactionAsync())
                    {
                        try
                        {
                            ...
                            if (booking == null)
                            {
                                throw new InvalidOperationException("Booking not found or you are not authorized to cancel it.");
                            }

                            // 🔄 ATOMIC OPERATION: Set RowVersion for booking optimistic concurrency control
                            if (rowVersion != null)
                            {
                                _context.Entry(booking).Property(b => b.RowVersion)
                                    .OriginalValue = rowVersion;
                            }

                            // 🔄 ATOMIC OPERATION: Set RowVersion for each seat being released
                            ...
                            // 🔄 ATOMIC SAVE: Updates seats and booking with RowVersion checks
                            await _context.SaveChangesAsync();
